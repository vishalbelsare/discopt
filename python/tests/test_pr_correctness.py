...
Four representative known-optima instances drawn from
:mod:`test_correctness`. These run on every PR (~10 s budget) so a
...
Nonconvex spatial B&B (e.g. ``circle_minlp``) is intentionally excluded
from the PR fast path: its 38 s wall time on its own broke the budget,
and it is fully exercised in the nightly correctness suite.
