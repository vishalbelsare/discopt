...
Out of scope in the current implementation and left to future work: robust / expected-information designs $\mathbb{E}_\theta[M]$ under parameter uncertainty in the spirit of cite:&Sandrin2025, mixed-integer batch designs selecting $N$ from a discrete candidate pool (naturally expressed in the core MINLP framework), and end-to-end JAX autograd through the joint batch objective.
